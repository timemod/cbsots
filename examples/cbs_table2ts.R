...
#
cbs_table2ts <- function(id, naam_kort, download, 
                         tijdreekscode_dir = "tijdreekscodes", 
                         ruwe_cbs_dir = "ruwe_cbs_data") {

  tijdreekscodefile <- file.path(tijdreekscode_dir, 
                                 paste0(naam_kort, "_code.xlsx"))
  
  # lees informatie in over de cbs-codering en de tijdreekscode-file
  cbs_code <- get_cbs_code(id, cache = !download)
  dimensies <- setdiff(names(cbs_code), "Topic")
  ts_code <- lees_tijdreekscodes(tijdreekscodefile = tijdreekscodefile,
                                 dimensies = dimensies)

  merge_code <- function(groep) {
    # Merge de opendata- en tijdreekscode
    
    cbs <- cbs_code[[groep]]
    ts  <- ts_code[[groep]]
    
    ts <- fix_ts(ts, cbs, groep)

    # combineer cbs met ts
    code <- merge(cbs, ts, sort = FALSE)
  ...
  code <- sapply(names(ts_code), FUN = merge_code, simplify = FALSE)
